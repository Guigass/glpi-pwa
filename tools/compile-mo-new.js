/**
 * Script Node.js para compilar arquivos .po para .mo na nova estrutura
 * Uso: node compile-mo-new.js
 * 
...
try {
    if (!fs.existsSync(localeDir)) {
        console.error(`Diretório não encontrado: ${localeDir}`);
        process.exit(1);
    }

    // Encontrar todos os arquivos .po (exceto .pot)
    const files = fs.readdirSync(localeDir);
    const poFiles = files.filter(f => f.endsWith('.po') && !f.endsWith('.pot'));
    ...
    for (const poFileName of poFiles) {
        ...
        const moFile = path.join(localeDir, basename + '.mo');
        ...
        console.log(`  -> ${basename}.mo (${Object.keys(entries).length} strings)`);
        compiled++;
    }
