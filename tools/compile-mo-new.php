<?php
/**
 * Script para compilar arquivos .po para .mo na nova estrutura
 * Uso: php compile-mo-new.php
 * 
 * Nova estrutura: locales/pt_BR.po -> locales/pt_BR.mo
 */
...
function poToMo($poFile, $moFile) {
    ...
    foreach ($originals as $orig) {
        $transOffset = $stringOffset + strlen($origStrings) + strlen($orig) + 1;
    }
    ...
    foreach ($originals as $i => $orig) {
        ...
        $origTable .= pack('V', strlen($orig));
        $origTable .= pack('V', $origOffset);
        $origOffset += strlen($orig) + 1;
        
        // Translation string table entry
        $transTable .= pack('V', strlen($trans));
...
$localeDir = $baseDir . '/locales';

if (!is_dir($localeDir)) {
    echo "Diretório não encontrado: $localeDir\n";
    exit(1);
}

// Encontrar todos os arquivos .po (exceto .pot)
...
foreach ($poFiles as $poFile) {
    ...
    // Gerar arquivo .mo na mesma pasta: locales/{locale}.mo
    $moFile = $localeDir . '/' . $basename . '.mo';
